...
This document establishes the formal mathematical architecture for the **Granas PV-HB** integrated state-space model. It derives the stochastic field equations governing photovoltaic energy generation, alongside the deterministic thermochemical kinetics of the Haber-Bosch reactor.
...
The control variable $u_{hb}(t) \in \mathcal{U}(x) \subset \mathbb{R}$ governs the power dispatched to the Haber-Bosch reactor, bounded by the instantaneous PV generation:
