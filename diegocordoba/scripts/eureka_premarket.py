...
SECTOR_PROXIES = {
    "MU":  0.25,   # Micron — semiconductor sibling
    "SMH": 0.20,   # VanEck Semis ETF — sector barometer
    "QQQ": 0.15,   # Broad tech direction
}
SNDK_WEIGHT = 0.40  # SNDK itself is the primary signal

DECAY_LAMBDA = 0.002250  # Locked from decay_parameter_lock.json
PREMARKET_HISTORY_PATH = os.path.expanduser(
    "~/.quantum_portfolio/premarket_history.json"
)
...
def fetch_premarket_data() -> dict:
    """
    Fetch pre-market / after-hours data for SNDK and sector proxies.
    ...
    tickers_to_fetch = [SNDK_TICKER] + list(SECTOR_PROXIES.keys()) + [VIX_TICKER]
    ...
    # Project SNXX open from SNDK gap (2x leverage)
    sndk_data = result.get(SNDK_TICKER, {})
...
def compute_sector_momentum(premarket_data: dict) -> float:
    """
    Weighted composite of pre-market changes across SNDK + sector proxies.
    ...
    # SNDK primary signal
    sndk_gap = premarket_data.get(SNDK_TICKER, {}).get("gap_pct")
    if sndk_gap is not None:
        weighted_sum += SNDK_WEIGHT * sndk_gap
        total_weight += SNDK_WEIGHT
...
def compute_decay_risk(markov_chain: MarkovDayChain, daily_returns: pd.Series) -> float:
    """
    Estimate today's decay risk for SNXX based on recent realized
...
def generate_premarket_signal() -> dict:
    ...
    # 2. Build Markov chain on full SNDK history
    logger.info("Building Markov chain on full SNDK history...")
    chain = build_full_history_chain(SNDK_TICKER)

    # 3. Compute components
    sector_score = compute_sector_momentum(pm_data)

    # Get SNDK daily returns for decay risk calc
    try:
        df = yf.download(SNDK_TICKER, period="30d", progress=False)
    ...
    # Gap signal: normalize SNDK gap to [-1, +1]
    sndk_gap = pm_data.get(SNDK_TICKER, {}).get("gap_pct", 0) or 0
    ...
    signal = {
        ...
        "sndk_price": pm_data.get(SNDK_TICKER, {}).get("price"),

        # Sector proxies
        "sector_score": sector_score,
        "mu_gap_pct":  (pm_data.get("MU", {}).get("gap_pct") or 0) * 100,
        "smh_gap_pct": (pm_data.get("SMH", {}).get("gap_pct") or 0) * 100,
        "qqq_gap_pct": (pm_data.get("QQQ", {}).get("gap_pct") or 0) * 100,
...
def format_premarket_message(signal: dict) -> str:
    ...
    exec_map = {
        "STRONG_BUY": "Buy at open or within first 15 min if SNXX opens within +/-0.5% of projected",
        ...
        "AVOID":      "DO NOT trade SNXX today — choppy regime + negative setup",
    }
    ...
    lines.append(f"\U0001f52e  Sector Pulse: {sector_label} "
                 ...
                 f"SMH {signal.get('smh_gap_pct', 0):+.1f}%, "
                 f"QQQ {signal.get('qqq_gap_pct', 0):+.1f}%)")
