...
def format_premarket_message(signal: dict) -> str:
    ...
    title = "🔮  EUREKA TOMORROW'S FORECAST (SCHD)" if is_pm else "🌅  EUREKA PRE-MARKET INTELLIGENCE"
    ...
    lines.append("=" * 59)
    lines.append(f"📅  {ts}")
    lines.append(f"📊  SCHD {price_label}: {signal.get('schd_gap_pct', signal.get('sndk_gap_pct', 0)):+.2f}%")
