...
class GridStabilizer:
    ...
    def __init__(self, market: str = "ERCOT", controller_type: str = "HJB",
        ...
        elif controller_type == "VZA400":
            if VZA400PIDESolver is None:
                raise ImportError("VZA400PIDESolver not available.")
            self.controller = VZA400PIDESolver(max_injection_mw=max_injection_mw)
    ...
    def run(self, duration_s: float = 600.0) -> SimulationResult:
        ...
        for step in range(n_steps):
            ...
            elif VZA400PIDESolver and isinstance(self.controller, VZA400PIDESolver):
                # We mock renewable capacity factor for the simulator (e.g. 0.8)
                control = self.controller.compute_dispatch(0.8, lmp)
...
def run_simulation(duration_s: float = 600.0, market: str = "ERCOT",
    ...
    """Run HJB vs PID vs No-Control vs VZA400 on identical disturbance profiles."""
    results = {}
    for ctrl in ["HJB", "PID", "VZA400", "NONE"]:
        if ctrl == "VZA400" and VZA400PIDESolver is None:
            continue
