...
from lib.prime_kernel.hopfield import HopfieldValueMemory
...
class GridFrequencyDynamics(HJBDynamics):
    ...
    def step(self, state: np.ndarray, control: float, dt: float) -> np.ndarray:
        ...
        # Inject cascade term for regularization valuation
        cascade = -0.5 * (df**2) if df < -0.5 else 0.0
        
        ddf = (P - self.D * df) / (2.0 * self.H) + cascade
    ...
    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        ...
        price = self.price_func(t) if hasattr(self, 'price_func') and self.price_func is not None else self._pen_coeff
        ...
        arbitrage_rev = -control * price * (1.0 / 3600.0) # revenue/cost from MW/s injection
        
        return freq_cost + energy_cost + penalty + arbitrage_rev
...
class RegimeSwitchingGridDynamics(HJBDynamics):
    ...
    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        ...
        pen_coeff  = price * (1.5 if s == 1 else 1.0)
        penalty    = pen_coeff * excess**2 if excess > 0 else 0.0
        
        arbitrage_rev = -control * price * (1.0 / 3600.0)
        return freq_cost + energy_cost + penalty + arbitrage_rev
...
class MultiAreaGridDynamics(HJBDynamics):
    ...
    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        ...
        price = self.price_func(t) if hasattr(self, 'price_func') and self.price_func is not None else 500.0
        penalty = price * float(np.sum(excess**2))
...
class BESSFrequencyDynamics(HJBDynamics):
    ...
    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        ...
        price = self.price_func(t) if hasattr(self, 'price_func') and self.price_func is not None else self._pen_coeff
        pen_freq    = price * excess**2 if excess > 0 else 0.0
        ...
        # Arbitrage revenue: inject power when price is high
        arbitrage_rev = -control * price * (1.0 / 3600.0)
        
        return freq_cost + pen_freq + doh_cost + thermal_pen + arbitrage_rev
...
class PerovskiteAnnealingDynamics(HJBDynamics):
    ...
    def running_cost(self, state, control, t: float = 0.0):
        _, _, temp = state
...
# ─────────────────────────────────────────────────────────────────────────────
# ICDN-Integrated Demand Elasticity Dynamics  (VZA-400 Pilot)
# ─────────────────────────────────────────────────────────────────────────────
class DemandElasticityDynamics(HJBDynamics):
    ...
    def step(self, state: np.ndarray, control: float, dt: float) -> np.ndarray:
        ...
        # Swing equation now incorporates Demand_Dev explicitly.
        ddf = (P_m + P_inj - demand_dev - self.D * df) / (2.0 * self.H)
        new_df = np.clip(df + ddf * dt, -2.0, 2.0)
        
        new_P_inj = np.clip(P_inj + control * dt, 0.0, self.max_inj)
        
        # Simple dynamics for P_m (assumed constant or slow-moving governor)
        new_P_m = P_m 
        
        # ICDN exact elasticity gradient drives demand deviation
        price = 500.0 # Could be dynamic or passed via t
    ...
    def diffusion(self, state: np.ndarray) -> np.ndarray:
        """Only frequency carries Brownian noise."""
        return np.array([self.sigma_ou, 0.0, 0.0, 0.0])
...
# ─────────────────────────────────────────────────────────────────────────────
# Thesis §7 — Bidimensional Sovereign Dynamics (Compound Real Option)
# ─────────────────────────────────────────────────────────────────────────────
class BidimensionalSovereignDynamics(HJBDynamics):
    """
    HJB dynamics modelling the optimal technological transition from conventional
    frequency regulation to the Granas-Sovereign perovskite + flow battery system.
    
    State (3D): [Δf (Hz), P_inj (MW), PCE (ratio)]
    Control: injection ramp rate (MW/s)
    
    The PCE (Power Conversion Efficiency) evolves stochastically via Geometric Brownian 
    Motion, driven by the SIBO / Self-Driving Lab progress.
    
    References:
    - Lasry & Lions (2007). "Mean field games".
      Provides the coupled PDE system foundation for verifying the SDL bridge's handling of cross-coupling terms.
    """

    def __init__(self,
                 market: ISOMarket = ISOMarket.CENACE,
                 mu_pce: float = 0.078,
                 sigma_pce: float = 0.124,
                 pce_0: float = 0.284):
        p = iso_params(market)
        self.nominal_freq  = p["nominal_hz"]
        self.H             = p["inertia_H"]
        ...
        # Technology evolution parameters (Thesis §7.3)
        self.mu_pce = mu_pce
        self.sigma_pce = sigma_pce
        self.pce_0 = pce_0
        
        # Real Option Parameters (Thesis §7.4)
        self.I_investment = 94_000_000.0  # MXN
        self.delta_pi = 3_250_000.0       # MXN per percentage point

    def state_dims(self) -> int: return 3

    def state_bounds(self) -> List[Tuple[float, float]]:
        return [
            (-2.0, 2.0),         # Δf
            (0.0, self.max_inj), # P_inj
            (0.20, 0.60)         # PCE ranges from 20% to 60%
        ]
    ...
    def step(self, state: np.ndarray, control: float, dt: float) -> np.ndarray:
        ...
        # PCE deterministic drift (GBM expected value)
        new_pce = np.clip(pce + self.mu_pce * pce * dt, 0.20, 0.60)
        
        return np.array([new_df, new_P, new_pce])

    def diffusion(self, state: np.ndarray) -> np.ndarray:
        """
        Diffusion vector: OU noise on frequency, GBM noise on PCE.
        """
        df, P, pce = state
        return np.array([self.sigma_ou, 0.0, self.sigma_pce * pce])
    ...
    def terminal_cost(self, state: np.ndarray) -> float:
        ...
        # Standard terminal grid cost
        grid_cost = 200.0 * df**2 + 0.1 * P
        
        # Intrinsic value of the Granas-Sovereign Real Option
        # Option payoff = max(0, ΔΠ * (PCE * 100) - I)
        option_payoff = max(0.0, self.delta_pi * (pce * 100.0) - self.I_investment)
        
        # Scale the payoff to match HJB cost magnitudes (scaled by 1e-6 for stability)
        scaled_payoff = option_payoff * 1e-6
        
        return grid_cost - scaled_payoff
...
class HJBSolver:
    ...
    def optimal_control(self, state: np.ndarray, t: float = 0.0) -> float:
        ...
        for u in self.control_grid:
            L      = self.dynamics.running_cost(state, u, t)
...
# ─────────────────────────────────────────────────────────────────────────────
# Thesis VZA-400 — VZA400Dynamics (frecuencia + PML reales)
# ─────────────────────────────────────────────────────────────────────────────
class VZA400Dynamics(HJBDynamics):
    """
    Dynamics for the VZA-400 node incorporating real CENACE data (frequency + PML).
    State: [Δf, P_inj, PML_USD]
    Control: injection ramp rate (MW/s)
    """
    ...
    def __init__(self,
        ...
        self._pen_coeff    = p["penalty_coeff"]
        self.max_inj       = max_inj_mw
        self.pml_mean      = pml_mean
        self.pml_reversion = pml_reversion
        self.pml_vol       = pml_vol

    def state_dims(self) -> int: return 3

    def state_bounds(self) -> List[Tuple[float, float]]:
        return [(-2.0, 2.0), (0.0, self.max_inj), (0.0, 500.0)]
    ...
    def step(self, state: np.ndarray, control: float, dt: float) -> np.ndarray:
        ...
        # Power injection dynamics
        new_P = np.clip(P + control * dt, 0.0, self.max_inj)
        
        # PML mean reversion (Ornstein-Uhlenbeck)
        dpml = self.pml_reversion * (self.pml_mean - pml)
        new_pml = np.clip(pml + dpml * dt, 0.0, 500.0)
        
        return np.array([new_df, new_P, new_pml])

    def diffusion(self, state: np.ndarray) -> np.ndarray:
        # Noise on frequency and PML
        return np.array([self.sigma_ou, 0.0, self.pml_vol])

    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        df, P, pml = state
        
        # Earn revenue from injection at current PML (scaled to seconds: PML is $/MWh)
        revenue = (P * pml) / 3600.0
        
        # Penalties and operational costs
        freq_cost = 100.0 * df**2
        energy_cost = 0.01 * abs(control)
        ...
        return freq_cost + energy_cost + penalty - revenue
...
# ─────────────────────────────────────────────────────────────────────────────
# VAZ-400 Charge Transport Layer HJB Dynamics
# ─────────────────────────────────────────────────────────────────────────────
class VAZ400ChargeTransportDynamics(HJBDynamics):
    """
    HJB formulation for the VAZ-400 Charge Transport Layer.
    Models the drift-diffusion and recombination of charge carriers in the VAZ-400 interface.
    
    State: [n (electron density), p (hole density), V_internal (potential)]
    Control: J_inj (injected current density)
    
    The value function ansatz is pinned to the boundary conditions to ensure stable gradients
    across the active area of the perovskite-TOPCon interface.
    """
    
    def __init__(self, mobility_e: float = 120.0, mobility_h: float = 80.0, T_c: float = 25.0):
        self.mu_e = mobility_e
        self.mu_h = mobility_h
        self.kT_q = 8.617e-5 * (T_c + 273.15)
        
        # Pinned boundary conditions
        self.n_0 = 1e16  # Cathode interface
        self.p_L = 1e18  # Anode interface
        self.V_built_in = 1.2
        
    def state_dims(self) -> int: return 3
    
    def state_bounds(self) -> List[Tuple[float, float]]:
        # Pinned boundary conditions enforced here
        return [
            (1e10, 1e20),   # n (cm^-3)
            (1e10, 1e20),   # p (cm^-3)
            (-0.5, 1.5)     # V_internal (Volts)
        ]
        
    def control_bounds(self) -> Tuple[float, float]:
        return (-40.0, 40.0) # J_inj in mA/cm^2
        
    def step(self, state: np.ndarray, control: float, dt: float) -> np.ndarray:
        n, p, V = state
        
        # Drift-diffusion approximation step
        E_field = (self.V_built_in - V) / 100e-7 # Assuming 100nm layer
        
        # Continuity equation simplified updates
        dn = (self.mu_e * n * E_field + self.mu_e * self.kT_q * (self.n_0 - n) / 100e-7) - 1e-10 * n * p
        dp = (self.mu_h * p * E_field + self.mu_h * self.kT_q * (self.p_L - p) / 100e-7) - 1e-10 * n * p
        dV = (control - 1.6e-19 * (dn - dp)) / 1e-9 # Capacitive charging
        
        # Enforce pinned boundaries dynamically
        new_n = np.clip(n + dn * dt, 1e10, 1e20)
        new_p = np.clip(p + dp * dt, 1e10, 1e20)
        new_V = np.clip(V + dV * dt, -0.5, 1.5)
        
        return np.array([new_n, new_p, new_V])
        
    def value_function_ansatz(self, state: np.ndarray) -> float:
        """
        Value function ansatz for the VAZ-400 Charge Transport Layer HJB.
        Assuming a quadratic-logarithmic form to penalize deviations from optimal carrier balance.
        """
        n, p, V = state
        # Analytical ansatz for the HJB value function V(x)
        # Pins optimal operation near maximum power extraction (balanced n, p, and optimal V)
        V_ansatz = 0.5 * (np.log10(n / max(self.n_0, 1e-10)))**2 + 0.5 * (np.log10(p / max(self.p_L, 1e-10)))**2 + 10.0 * (V - self.V_built_in)**2
        return float(V_ansatz)
        
    def running_cost(self, state: np.ndarray, control: float, t: float = 0.0) -> float:
        n, p, V = state
        # Non-radiative recombination penalty
        recombination_cost = 1e-14 * n * p 
        # Joule heating / resistive loss from control current
        resistive_cost = 0.05 * control**2
        return float(recombination_cost + resistive_cost)
        
    def terminal_cost(self, state: np.ndarray) -> float:
        # Pinned boundary penalty at terminal time based on ansatz
        return self.value_function_ansatz(state)
