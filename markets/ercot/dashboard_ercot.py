...
# ============================================================
#  PRIMEnergeia — ERCOT Market Dashboard
#  Hamilton-Jacobi-Bellman Optimal Frequency Control System
#  Electric Reliability Council of Texas
# ============================================================

st.set_page_config(
    page_title="PRIMEnergeia ERCOT | Grid Control",
    page_icon="⚡",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# --- PREMIUM CSS (ERCOT: Texas Orange Accent) ---
st.markdown("""
<style>
...
[data-testid="stMetric"] {
    ...
    border-radius: 8px;
    padding: 18px 20px;
    box-shadow: 0 4px 20px rgba(255, 107, 53, 0.04);
}
...
.stTabs [aria-selected="true"] {
    background: linear-gradient(135deg, #ff6b3522, #cc440022);
...
.kpi-highlight {
    background: linear-gradient(135deg, #1a0d00, #221400);
...
# ============================================================
#  SIMULATION ENGINE — ERCOT Grid Physics
# ============================================================

@st.cache_data(ttl=2)
def generate_simulation_state():
    """Full ERCOT grid state simulation with physics-accurate data."""
    np.random.seed(int(time.time()) % 10000)
    now = datetime.now()

    # --- Grid Frequency Physics (Swing Equation — ERCOT) ---
    f_nominal = 60.0
    H = 4.5   # Lower inertia — high wind/solar in ERCOT
    D = 1.8
    dt = 0.01
    n_steps = 600
    ...
    p_electrical = np.ones(n_steps) + np.random.normal(0, 0.010, n_steps)
    p_electrical[200:230] += 0.05  # Sudden load event (heat wave)
    p_electrical[420:445] += 0.08  # Larger disturbance (generator trip)
    ...
    # --- LMP Market Dynamics (ERCOT) ---
    hours = np.arange(0, 24, 0.25)
    lmp_base = 35 + 30 * np.sin((hours - 10) * np.pi / 12)
    lmp_noise = np.cumsum(np.random.normal(0, 2.0, len(hours))) * 0.4
    # ERCOT scarcity pricing: extreme spikes up to $5,000/MWh
    lmp_spikes = np.where(np.random.rand(len(hours)) > 0.96, np.random.uniform(500, 5000, len(hours)), 0)
    lmp_prices = np.clip(lmp_base + lmp_noise + lmp_spikes, -50, 5000)

    # --- Voltage & Power Factor ---
    v_a = 345.0 + np.random.normal(0, 0.5)
    v_b = 345.0 + np.random.normal(0, 0.6)
    v_c = 345.0 + np.random.normal(0, 0.4)
    pf = 0.97 + np.random.normal(0, 0.006)

    # --- THD (IEEE 519) ---
    harmonics = {3: 0.07 + np.random.normal(0, 0.012),
                 5: 0.05 + np.random.normal(0, 0.009),
                 7: 0.03 + np.random.normal(0, 0.006),
                 9: 0.015 + np.random.normal(0, 0.003),
                 11: 0.009 + np.random.normal(0, 0.002),
                 13: 0.005 + np.random.normal(0, 0.001)}
    thd = np.sqrt(sum(v**2 for v in harmonics.values())) * 100

    # --- Capital Recovery ---
    optimal_mw = 120.0 * np.maximum(0, np.sin((hours - 6) * np.pi / 12))
    legacy_loss = np.random.uniform(0.72, 0.85, len(hours))
    actual_mw = optimal_mw * legacy_loss
    actual_mw *= np.where(lmp_prices > 500, np.random.uniform(0.55, 0.70, len(hours)), 1.0)
    actual_mw *= np.random.normal(1.0, 0.018, len(hours))
    actual_mw = np.maximum(0, actual_mw)
    delta_mw = np.maximum(0, optimal_mw - actual_mw)
    capital_per_interval = delta_mw * lmp_prices * 0.25
    capital_cumulative = np.cumsum(capital_per_interval)

    # --- Node Network Status ---
    nodes = [
        {"id": "HOU-345-01", "loc": "Houston Central", "region": "Houston", "cap": 120, "status": "MASTER", "f": 60.0 + np.random.normal(0, 0.009)},
        {"id": "HOU-345-02", "loc": "Baytown", "region": "Houston", "cap": 100, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.011)},
        {"id": "HOU-138-03", "loc": "Galveston", "region": "Houston", "cap": 60, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.013)},
        {"id": "NTH-345-01", "loc": "Dallas–Fort Worth", "region": "North", "cap": 120, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.010)},
        {"id": "NTH-345-02", "loc": "Denton", "region": "North", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.012)},
        {"id": "NTH-138-03", "loc": "Waco", "region": "North", "cap": 60, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.015)},
        {"id": "STH-345-01", "loc": "San Antonio", "region": "South", "cap": 100, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.010)},
        {"id": "STH-345-02", "loc": "Corpus Christi", "region": "South", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.012)},
        {"id": "STH-138-03", "loc": "Laredo", "region": "South", "cap": 50, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.016)},
        {"id": "AUS-345-01", "loc": "Austin", "region": "South", "cap": 100, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.009)},
        {"id": "AUS-138-02", "loc": "Georgetown", "region": "South", "cap": 60, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.013)},
        {"id": "AUS-138-03", "loc": "Round Rock", "region": "South", "cap": 50, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.015)},
        {"id": "WST-345-01", "loc": "Midland–Odessa", "region": "West", "cap": 100, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.014)},
        {"id": "WST-345-02", "loc": "Abilene", "region": "West", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.013)},
        {"id": "WST-138-03", "loc": "San Angelo", "region": "West", "cap": 60, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.015)},
        {"id": "FWS-345-01", "loc": "El Paso", "region": "Far West", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.016)},
        {"id": "FWS-138-02", "loc": "Pecos", "region": "Far West", "cap": 60, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.018)},
        {"id": "CST-345-01", "loc": "Victoria", "region": "Coast", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.012)},
        {"id": "CST-138-02", "loc": "Bay City", "region": "Coast", "cap": 60, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.014)},
        {"id": "CST-138-03", "loc": "Freeport", "region": "Coast", "cap": 50, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.016)},
        {"id": "PNH-345-01", "loc": "Amarillo", "region": "Panhandle", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.015)},
        {"id": "PNH-138-02", "loc": "Lubbock", "region": "Panhandle", "cap": 60, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.016)},
        {"id": "EST-345-01", "loc": "Beaumont", "region": "East", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.012)},
        {"id": "EST-345-02", "loc": "Tyler", "region": "East", "cap": 80, "status": "ONLINE", "f": 60.0 + np.random.normal(0, 0.013)},
        {"id": "EST-138-03", "loc": "Lufkin", "region": "East", "cap": 50, "status": "STANDBY", "f": 60.0 + np.random.normal(0, 0.017)},
    ]

    load_values = [
        np.random.randint(75, 92), np.random.randint(50, 78), np.random.randint(30, 60),
        np.random.randint(65, 90), np.random.randint(45, 72), np.random.randint(30, 58),
        np.random.randint(55, 82), np.random.randint(42, 70), np.random.randint(25, 55),
        np.random.randint(60, 85), np.random.randint(35, 62), np.random.randint(25, 50),
        np.random.randint(50, 78), np.random.randint(42, 68), np.random.randint(30, 58),
        np.random.randint(48, 75), np.random.randint(28, 55),
        np.random.randint(45, 72), np.random.randint(35, 62), np.random.randint(25, 50),
        np.random.randint(50, 78), np.random.randint(35, 62),
        np.random.randint(48, 75), np.random.randint(45, 72), np.random.randint(25, 50),
    ]
    for i, node in enumerate(nodes):
        node["load"] = load_values[i]
    ...
    return {
        ...
        "lmp_prices": lmp_prices, "hours": hours,
...
with h1:
    st.markdown("# ⚡ PRIMEnergeia — ERCOT")
    st.caption("HAMILTON-JACOBI-BELLMAN OPTIMAL FREQUENCY CONTROL | TEXAS INTERCONNECT")
with h2:
    ...
    st.caption(f"Protocol: PRIME-HJB-v8.0-ERCOT")
with h3:
    ...
    st.caption(f"Latency: 0.{np.random.randint(3,9)}ms | Uptime: 99.97%")
...
k1.metric("FREQUENCY", f"{f:.4f} Hz", f"Δ {f - 60.0:+.4f}")
k2.metric("RoCoF", f"{state['rocof']:+.5f} Hz/s", "Swing Eq.")
k3.metric("VOLTAGE Φ-A", f"{state['v_a']:.1f} kV", "345 kV Nom.")
k4.metric("THD", f"{state['thd']:.2f} %", f"{'✓ IEEE 519' if state['thd'] < 5 else '⚠ Exceeds'}")
k5.metric("COS φ", f"{state['pf']:.4f}", "Unity Target")
k6.metric("HOUR OF DAY", now.strftime("%H:%M"), "Live")
k7.metric("RESCUED ACC.", f"${state['capital_total']:,.0f}", "USD / day")
...
# ═══════════════════════════════════════════════
with tab1:
    st.markdown("<div class='section-header'>REAL-TIME SWING EQUATION DYNAMICS — ERCOT ISLANDED GRID</div>", unsafe_allow_html=True)

    st.markdown("""
    <div class='math-block'>
    <strong>Swing Equation:</strong>&nbsp;&nbsp; M<sub>eff</sub> · (df/dt) = P<sub>m</sub> − P<sub>e</sub> − D · (f − f<sub>0</sub>)<br>
    <strong>Where:</strong>&nbsp;&nbsp; M<sub>eff</sub> = 2H + u<sub>inertia</sub> &nbsp;|&nbsp; H = 4.5s &nbsp;|&nbsp; D = 1.8 &nbsp;|&nbsp; dt = 10ms<br>
    <strong>Note:</strong>&nbsp;&nbsp; ERCOT operates as an islanded grid — no AC ties to Eastern or Western Interconnects
    </div>
    """, unsafe_allow_html=True)
    ...
    fig_phys.add_hline(y=59.97, line_dash="dot", line_color="#ff4b4b", annotation_text="NERC Penalty", row=1, col=1)
    fig_phys.add_hline(y=60.03, line_dash="dot", line_color="#ff4b4b", row=1, col=1)
    ...
    pc1.metric("INERTIA CONSTANT H", "4.5 s", "Lower (high renewables)")
    pc2.metric("DAMPING COEFF D", "1.8 p.u.", "ERCOT Calibrated")
...
with tab2:
    ...
    fig_hjb.add_trace(go.Scatter(x=sg, y=state["V_function"], name="V(x)",
        line=dict(color="#ff6b35", width=3), fill='tozeroy', fillcolor='rgba(255,107,53,0.06)'), row=1, col=1)
...
# ═══════════════════════════════════════════════
with tab3:
    st.markdown("<div class='section-header'>FIDUCIARY CAPITAL RECOVERY ENGINE — ERCOT LMP MARKET</div>", unsafe_allow_html=True)

    st.markdown("""
    <div class='math-block'>
    <strong>Recovery Formula:</strong>&nbsp;&nbsp; Σ [ (P*<sub>optimal</sub> − P<sub>actual</sub>) × LMP × Δt ]<br>
    <strong>Settlement:</strong>&nbsp;&nbsp; 5-min ERCOT Real-Time Market &nbsp;|&nbsp; <strong>Royalty:</strong>&nbsp;&nbsp; 25% of rescued capital<br>
    <strong>Scarcity Price Cap:</strong>&nbsp;&nbsp; $5,000 / MWh (ERCOT System-Wide Offer Cap)
    </div>
    """, unsafe_allow_html=True)
    ...
    with fc2:
        ...
        st.markdown(f"""<div class='kpi-highlight'>
            <div class='kpi-value' style='color:#ff6b35;'>${client_net:,.0f}</div>
    ...
    fig_fin.add_vline(x=curr_h, line_dash="dash", line_color="#ff6b35", opacity=0.7, annotation_text="LIVE", annotation_position="top left")

    st.plotly_chart(fig_fin, use_container_width=True)

    # LMP Market
    st.markdown("<div class='section-header'>LMP MARKET DYNAMICS — ERCOT NODE</div>", unsafe_allow_html=True)
    fig_lmp = go.Figure()
    fig_lmp.add_trace(go.Scatter(x=h, y=state["lmp_prices"], name="LMP (USD/MWh)",
        line=dict(color="#ff6b35", width=2), fill='tozeroy', fillcolor='rgba(255,107,53,0.06)'))
    fig_lmp.add_hline(y=500, line_dash="dash", line_color="#ff4b4b", annotation_text="Scarcity Threshold")
    fig_lmp.update_layout(template="plotly_dark", height=280,
        paper_bgcolor="#050810", plot_bgcolor="#0a0f1a",
        margin=dict(l=60, r=20, t=20, b=40),
        font=dict(family="JetBrains Mono", size=11, color="#6b7fa3"))
    fig_lmp.update_xaxes(gridcolor="#1a2744")
    fig_lmp.update_yaxes(gridcolor="#1a2744")

    curr_h = now.hour + now.minute/60.0
    fig_lmp.add_vline(x=curr_h, line_dash="dash", line_color="#ff6b35", opacity=0.7, annotation_text="LIVE", annotation_position="top left")
...
# ═══════════════════════════════════════════════
with tab4:
    st.markdown("<div class='section-header'>HARMONIC DISTORTION ANALYSIS — IEEE 519 COMPLIANCE</div>", unsafe_allow_html=True)

    st.markdown("""
    <div class='math-block'>
    <strong>THD Calculation:</strong>&nbsp;&nbsp; THD = √(Σ V<sub>h</sub>²) / V<sub>1</sub> × 100%<br>
    <strong>IEEE 519 Limit:</strong>&nbsp;&nbsp; THD ≤ 5.0% &nbsp;|&nbsp; Individual harmonic ≤ 3.0%
    </div>
    """, unsafe_allow_html=True)
    ...
    fig_harm.add_trace(go.Bar(
        ...
        marker=dict(color=["#ff4b4b" if v > 3 else "#ff6b35" for v in harm_values],
                    line=dict(color="#2a1a10", width=1)),
        text=[f"{v:.2f}%" for v in harm_values], textposition='outside',
        textfont=dict(family="JetBrains Mono", size=12, color="#e0e6ed")
    ))
    fig_harm.add_hline(y=3.0, line_dash="dash", line_color="#ff4b4b", annotation_text="IEEE 519 Limit (3%)")
    ...
    fig_3ph.add_trace(go.Scatter(x=t_wave*1000, y=phase_a, name="Phase A (filtered)", line=dict(color="#ff6b35", width=2.5)))
    ...
    vc4.metric("IMBALANCE", f"{abs(state['v_a']-state['v_b'])*100/345:.3f} %", "< 2% req.")
...
# ═══════════════════════════════════════════════
with tab5:
    st.markdown("<div class='section-header'>ERCOT GRID — 25-NODE SOVEREIGN NETWORK (TEXAS INTERCONNECT)</div>", unsafe_allow_html=True)
    ...
    for region_name, region_nodes in regions.items():
        st.markdown(f"<p style='font-family:JetBrains Mono; font-size:11px; color:#ff6b35; letter-spacing:2px; margin:16px 0 8px 0;'>▸ {region_name.upper()}</p>", unsafe_allow_html=True)
        for i in range(0, len(region_nodes), 3):
            ...
            for j, col in enumerate(cols):
                if i + j < len(region_nodes):
                    ...
                    with col:
                        st.markdown(f"""
                        <div style='background: linear-gradient(135deg, #0d1520, #111b2a); border: 1px solid #2a1a10;
                            ...
                            <div style='display:flex; justify-content:space-between; align-items:center;'>
                                ...
                                <span style='font-family:JetBrains Mono; font-size:16px; color:#ff6b35;'>{node["f"]:.3f} Hz</span>
                            </div>
    ...
    ns4.metric("TOTAL CAPACITY", f"{total_cap:,} MW", "25-Node Aggregate")
...
# ═══════════════════════════════════════════════
with tab6:
    st.markdown("<div class='section-header'>SYSTEM AUDIT LOG — NERC/ERCOT COMPLIANCE RECORD</div>", unsafe_allow_html=True)
    ...
    audit_data = {
        ...
        "Event": [
            "HJB convergence verified", "LMP spike detected ($4,200/MWh)", "Inertia injection: 2.8 p.u.",
            "Frequency stabilized: 59.998 Hz", "Node DFW-345 sync confirmed", "Capital checkpoint: $18,480",
            "THD measurement: 1.92%", "IEEE 519: COMPLIANT", "RoCoF within limits: +0.0015",
            "Node HOU-345 load rebalanced", "LMP normalized: $38/MWh", "HJB re-solved (state change)",
            "Swing Equation integration OK", "Phase imbalance: 0.02%", "Capital checkpoint: $42,940",
            "DRL actor-critic weight update", "Network heartbeat: all nodes", "LMP arbitrage captured: $8,200",
...
with fc3:
    st.caption("Energy Sovereignty for Texas ⚡🇺🇸")
    st.caption(f"Build: PRIME-HJB-v8.0-ERCOT | {now.strftime('%Y')}")
