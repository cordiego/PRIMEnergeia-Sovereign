...
st.caption("Offshore & Onshore Wind | Green H₂ Integration | 15 MW Direct-Drive Turbines | PRIMEnergeia S.A.S.")
...
t5.metric("H₂ Output", f"{td['h2']:,} kg/day")
t6.metric("Direct Jobs", f"{td['jobs']}")

st.divider()

tab1, tab2, tab3, tab4 = st.tabs(["📈 Power Curve", "☀️🌀 Hybrid", "💰 Revenue", "🗺️ Wind Map"])
...
with tab2:
    ...
    total = solar + wind
    h2_rate = total * 18  # kg H2 per MW per hour (PEM electrolyzer)

    fig2 = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.1,
        subplot_titles=(f"Combined Output — {tier}", "Green H₂ Production Rate"))
    ...
    fig2.add_trace(go.Bar(x=hours, y=h2_rate, name="H₂ (kg/hr)",
...
with tab3:
    ...
    h2_rev = td['h2'] * 365 * 4 / 1e6  # $4/kg H2
    rec_rev = annual_gwh * 5 / 1000  # $5/MWh REC
    ...
    fig3.add_trace(go.Scatter(x=years, y=np.cumsum(np.full_like(years, energy_rev + h2_rev, dtype=float)),
        name="+ H₂ Sales", fill="tonexty", fillcolor="rgba(0,200,120,0.15)", line=dict(color="#00c878")))
...
with tab4:
    st.markdown("""
    ### 🗺️ Target Regions for PRIM Wind
