...
with tab1:
    materials = [
        ...
        ("PbI₂ (Perovskite)", 1.2, 97.5, 45.0, "#00c878"),
    ...
    value_kg = [m[3] for m in materials]
    colors = [m[4] for m in materials]
    value_per_mod = [m[1] * m[2] / 100 * m[3] / 100 for m in materials]  # $ per module (assuming 20kg module)
...
with tab3:
    scale = st.radio("Processing Scale", ["Pilot (1K modules/yr)", "Regional (100K/yr)", "Industrial (1M/yr)"], horizontal=True, index=2)
    scale_data = {
        "Pilot (1K modules/yr)": {"modules": 1000, "capex": 0.5, "opex_per_mod": 12, "rev_per_mod": 18.5},
...
with tab4:
    ...
    i1.metric("CO₂ Avoided", "3.2 kg/mod")
